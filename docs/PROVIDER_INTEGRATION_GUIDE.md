...
| --- | --- | --- | --- | --- | --- |
| 0x | Swap API v2 AllowanceHolder firm quote | 必需 `providerKeys.0x`，0x Dashboard | 1, 10, 56, 130, 137, 143, 146, 999, 5000, 8453, 9745, 42161, 43114, 59144, 80094, 534352 | `0xeeee…` | approval spender 与 `transaction.to` 是两个字段；绝不能 approve Settler |
| 1inch | Classic Swap v6.1 | 必需 `providerKeys.1inch`，1inch Business Portal | 1, 10, 56, 130, 137, 143, 146, 999, 8453, 42161, 43114, 59144 | `0xeeee…` | Bearer；slippage 是百分数；当前使用官方 Router v6 地址 |
| KyberSwap | Aggregator legacy public route/build | 无 key 可用；`providerKeys.kyber` 可选 | 1, 10, 56, 130, 137, 143, 146, 999, 5000, 8453, 9745, 42161, 43114, 59144, 80094 | `0xeeee…` | route/build 必须连续完成；route 最多缓存约 5–10 秒；公开网关限流较严 |
| Barter | Router API route → swap | 必需 `providerKeys.barter`，向 Barter 获取 | 1, 8453, 42161 | 未形成可靠公开契约 | 每次调用唯一 `X-Request-Id`；deadline 是毫秒；`minReturn` 至少为 quote 的 98% |
| Bebop | PMM/RFQ v3 self-execution | demo 可免 key；`providerKeys.bebop` 可选、生产建议申请 | 1, 10, 56, 137, 999, 8453, 42161 | `0xeeee…` | 地址需 EIP-55 checksum；使用 firm `minimumAmount` 和秒级 expiry |
| Enso | Shortcuts Route，`router` strategy | 必需 `providerKeys.enso`，Enso Dashboard | 1, 10, 56, 130, 137, 143, 146, 999, 8453, 9745, 42161, 43114, 59144, 80094 | `0xeeee…` | POST body 中 token/amount 是数组；v1 主动拒绝跨链 route |
| HyperBloom | HyperEVM Swap `/quote` | 必需 `providerKeys.hyperBloom`，联系团队 | 999 | `0xeeee…` | `api-key` header；slippage 是小数比例；仅 HyperEVM |
...
| Odos | SOR quote v2 → assemble | 官方公开 API 无认证；`providerKeys.odos` 仅作为可选透传 | 1, 10, 56, 130, 137, 146, 5000, 8453, 42161, 43114, 59144, 534352 | 零地址 | quote 的 `pathId` 再 assemble；approval target 取组装交易 Router |
| OogaBooga | Swap API | 必需 `providerKeys.oogaBooga`，联系团队 | 999, 80094 | 零地址 | 只支持 `/tokens` 白名单；router/executor 动态返回，不硬编码 |
...
3. key 只存在服务端 `config.json`（字段与迁移表见 [配置说明](CONFIGURATION.md)），不返回 API、不写日志、不进 fixture。
...
- 前提：创建 0x application 并配置 `providerKeys.0x`；请求带 `0x-api-key` 和强制的 `0x-version: v2`。
...
- 前提：在 1inch Business Portal 创建 application/key，配置 `providerKeys.1inch`，使用 `Authorization: Bearer`。
...
- 前提：当前代码保留可匿名访问的 `aggregator-api.kyberswap.com/{chain}/api/v1`。`providerKeys.kyber` 是可选 client identity；缺失时仍参与，但公开网关配额更低。Kyber 新商业 gateway/API key 不在未确认完整 endpoint 契约前混入此 adapter。
...
- 前提：配置 `providerKeys.barter`，使用 Bearer；每个 HTTP 调用都生成新的 UUID `X-Request-Id`。
...
- 前提：demo 可不带 key，但配额和市场覆盖受限；生产应申请 key 并配置 `providerKeys.bebop`，adapter 会用 Bearer 透传。
...
- 前提：Enso Dashboard 申请 `providerKeys.enso`，Bearer 认证；官方默认限流需按账户核对。
...
- 前提：联系 HyperBloom 获取 key，配置 `providerKeys.hyperBloom`，header 名为 `api-key`。
...
- 前提：无认证；但本项目需要该链 RPC 读取 sell token `decimals()`，例如显式 `chains.999.rpcUrl`，或由 `alchemyApiKey` 自动生成的 HyperEVM endpoint。
...
- 前提：官方公共 `api.odos.xyz` 路径不要求 access key，因此无 key 也参与。为兼容已有部署，若配置 `providerKeys.odos`，adapter 只以 `x-api-key` 可选透传；它不是参与门槛。
...
- 前提：联系团队申请 key，配置 `providerKeys.oogaBooga`，Bearer 认证。
...
- 前提：在 OKX Developer Portal 创建项目和 API credentials；必需 `providerKeys.okx`、`okxSecretKey`、`okxPassphrase`。`okxProjectId` 若存在则附加，但官方 swap request 示例不把它作为必需签名头。
...
- 0x/1inch/Enso 等动态交易目标嵌入 MetaRouter 后的主网 fork 行为。Rust 与合约均 permissionless，不登记 allowlist；provider 原生地址约束和完整仿真仍必须通过，API 返回本身不是可执行证明。
